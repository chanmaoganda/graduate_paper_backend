...
pub fn get_teacher_apis() -> actix_web::Scope {
    let query_id_api = web::resource(QUERY_ID_ENDPOINT)
        .route(web::get().to(get_services::get_teacher_by_id));

    let list_api = web::resource(LIST_ENDPOINT)
        .route(web::get().to(get_services::list_all_teachers));

    let register_api = web::resource(REGISTER_ENDPOINT)
        .route(web::post().to(post_services::register_teacher));
...
mod get_services {
    ...
    pub async fn get_teacher_by_id(teacher_id: web::Query<QueryById>, pool: web::Data<Pool>) -> impl Responder {
...
mod post_services {
    ...
    pub async fn register_teacher(teacher: web::Query<Teacher>, pool: web::Data<Pool>) -> impl Responder {
        let client = pool.get().await.unwrap();

        log::debug!("create teacher");

        let Teacher { teacher_id, name, email } = teacher.into_inner();

        let sql = if let Some(email) = email {
            format!("INSERT INTO {TEACHER_TABLE} (name, teacher_id, email) VALUES ('{}', '{}', '{}');", name, teacher_id, email)
        } else {
            format!("INSERT INTO {TEACHER_TABLE} (name, teacher_id) VALUES ('{}', '{}');", name, teacher_id)
        };
